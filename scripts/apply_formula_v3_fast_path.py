...
def main() -> int:
    ...
    if "    Roc {" not in text or "    Mom {" not in text:
        text = replace_once(
            text,
    ...
    if '"ROC" if args.len() == 2 && is_close(args[0])' not in text:
        text = replace_once(
            text,
    ...
    if "CanonicalFormula::Roc { period } =>" not in text:
        text = replace_once(
            text,
    ...
    if "CanonicalFormula::Roc { period } =>" not in text or "CanonicalFormula::Mom { period } =>" not in text:
        raise RuntimeError("canonical executor is missing ROC/MOM fast paths")
