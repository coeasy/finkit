...
build_native() {
  echo "[build-installer] cargo build --release -p alpha-ta-ffi"
...
stage_libs() {
  ...
  rm -rf "${stage}"
  mkdir -p "${stage}/lib" "${stage}/include" "${stage}/bin" "${stage}/share/alpha-ta"
  ...
  case "$(uname -s)" in
    MINGW*|CYGWIN*|MSYS*)
      cp "${ROOT}/target/release/alpha_ta_ffi.dll"      "${stage}/bin/"
      cp "${ROOT}/target/release/alpha_ta_ffi.dll.lib"  "${stage}/lib/"
      ;;
    Darwin*)
      cp "${ROOT}/target/release/libalpha_ta_ffi.dylib" "${stage}/lib/"
      install_name_tool -id "@rpath/libalpha_ta_ffi.dylib" "${stage}/lib/libalpha_ta_ffi.dylib" 2>/dev/null || true
      ;;
    Linux*)
      cp "${ROOT}/target/release/libalpha_ta_ffi.so"    "${stage}/lib/"
      ;;
  esac

  # License files for the system installer payload.
  if [[ -f "${ROOT}/LICENSE" ]]; then
    cp "${ROOT}/LICENSE" "${stage}/share/alpha-ta/LICENSE"
  fi
}
...
build_msi() {
  ...
  light.exe -ext WixUIExtension -out "${DIST}/alpha-ta-${VERSION}-${RUST_TRIPLE}-pc-windows-msvc.msi" \
...
build_deb() {
  ...
  local pkgroot="${DIST}/_deb/AlphaTA_${VERSION}"
  rm -rf "${pkgroot}"
  mkdir -p "${pkgroot}/DEBIAN" \
           "${pkgroot}/usr/lib" \
           "${pkgroot}/usr/include/alpha-ta" \
           "${pkgroot}/usr/share/doc/alpha-ta"

  cp -r "${DIST}/_stage/lib/."   "${pkgroot}/usr/lib/"
  cp    "${DIST}/_stage/include/."  "${pkgroot}/usr/include/alpha-ta/"
  cp    "${DIST}/_stage/share/alpha-ta/LICENSE" \
        "${pkgroot}/usr/share/doc/alpha-ta/copyright"
...
Maintainer: AlphaTA Contributors <[email protected]>
Depends: libc6
...
EOF
  ...
  fakeroot dpkg-deb --build "${pkgroot}" \
      "${DIST}/alpha-ta_${VERSION}_$(dpkg --print-architecture).deb"
}
...
build_rpm() {
  ...
  local src_tar="${topdir}/SOURCES/alpha-ta-${VERSION}.tar.gz"
  tar -C "${DIST}/_stage" -czf "${src_tar}" .

  cat > "${topdir}/SPECS/alpha-ta.spec" <<EOF
Name:           alpha-ta
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        AlphaTA — high-performance financial technical analysis library
License:        MIT OR Apache-2.0
URL:            https://github.com/alpha-ta-rs/alpha-ta
Source0:        alpha-ta-${VERSION}.tar.gz
BuildArch:      $(uname -m)
%description
...
mkdir -p %{buildroot}/usr/include/alpha-ta
mkdir -p %{buildroot}/usr/share/doc/alpha-ta
cp -r lib/.  %{buildroot}/usr/lib/
cp -r include/. %{buildroot}/usr/include/alpha-ta/
cp share/alpha-ta/LICENSE %{buildroot}/usr/share/doc/alpha-ta/
%files
/usr/lib/libalpha_ta_ffi.so
%dir /usr/include/alpha-ta
/usr/include/alpha-ta/*.h
/usr/include/alpha-ta/*.hpp
/usr/share/doc/alpha-ta/LICENSE
%changelog
* $(date '+%a %b %d %Y') AlphaTA Contributors - ${VERSION}-1
- Automated build via packaging/build-installer.sh
EOF

  rpmbuild --define "_topdir ${topdir}" -ba "${topdir}/SPECS/alpha-ta.spec"
...
build_pkg() {
  ...
  mkdir -p "${pkgroot}/usr/local/lib" \
           "${pkgroot}/usr/local/include/alpha-ta" \
           "${pkgroot}/usr/local/share/alpha-ta"

  cp -r "${DIST}/_stage/lib/."   "${pkgroot}/usr/local/lib/"
  cp    "${DIST}/_stage/include/."  "${pkgroot}/usr/local/include/alpha-ta/"
  cp    "${DIST}/_stage/share/alpha-ta/LICENSE" \
        "${pkgroot}/usr/local/share/alpha-ta/LICENSE"

  local out="${DIST}/alpha-ta-${VERSION}-${RUST_TRIPLE}-apple-darwin.pkg"
  pkgbuild \
    --root "${pkgroot}" \
    --identifier "com.alpha-ta.lib" \
    --version "${VERSION}" \
    --install-location "/" \
...
build_dmg() {
  ...
  rm -rf "${staging}"
  mkdir -p "${staging}"
  cp "${DIST}"/alpha-ta-*-apple-darwin.pkg "${staging}/"

  local rw="${staging}/alpha-ta-rw.dmg"
  hdiutil create -ov -fs HFS+ -srcfolder "${staging}" -volname "AlphaTA ${VERSION}" \
      "${rw}"
  hdiutil convert "${rw}" -format UDZO -o "${DIST}/alpha-ta-${VERSION}-${RUST_TRIPLE}-apple-darwin.dmg"
  rm -f "${rw}"
}
