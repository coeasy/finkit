...
impl StreamingIndicator<f64, KstOutput> for StreamingKst {
    ...
    fn next(&mut self, input: f64) -> Option<KstOutput> {
        ...
        let s1 = self
            .sma1
            .next(r1.map(|v| if v.is_nan() { 0.0 } else { v }).unwrap_or(0.0));
        let s2 = self
            .sma2
            .next(r2.map(|v| if v.is_nan() { 0.0 } else { v }).unwrap_or(0.0));
        let s3 = self
            .sma3
            .next(r3.map(|v| if v.is_nan() { 0.0 } else { v }).unwrap_or(0.0));
        let s4 = self
            .sma4
            .next(r4.map(|v| if v.is_nan() { 0.0 } else { v }).unwrap_or(0.0));

        let all_ready = r1.is_some()
            && r2.is_some()
            && r3.is_some()
            && r4.is_some()
            && s1.is_some()
...
mod tests {
    ...
    fn test_streaming_vs_batch_convergence() {
        ...
        let batch =
            crate::indicators::momentum_ext::kst(&data, 10, 15, 20, 30, 10, 10, 10, 15, 9).unwrap();
