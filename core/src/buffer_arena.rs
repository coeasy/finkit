...
//! coupling their numerical kernels. Buffers are keyed by logical length,
//! zeroed (or explicitly filled) when checked out, and retained only within
//! configured count/byte limits.
...
impl BufferArena {
    ...
    pub fn take_filled(&mut self, len: usize, fill: f64) -> Vec<f64> {
        ...
        let mut buffer = if let Some(buffer) = cached {
