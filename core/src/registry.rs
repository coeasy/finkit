...
pub fn builtin_function_registry() -> FunctionRegistry {
    ...
    let specs = [
        ...
        // TA-Lib 0.7/0.8 formula bridge (M0-1 surface). These names are callable
        // from the formula DSL and now have a compiled-plan kernel that delegates
        // to the exact `canonical_*` implementation, so the plan path is fully
        // connected for them. `ACCBANDS` and `MAMA` were already registered.
        FunctionSpec {
            name: "AC",
            aliases: &[],
            category: FunctionCategory::Volume,
