...
impl PineRuntime {
  ...
  pub fn register_series(&mut self, name: &str, values: &[f64]) {
    let series: Vec<SeriesValue> = values
      .iter()
      .map(|&v| SeriesValue::new(v))
      .collect();
  ...
  pub fn set(&mut self, name: &str, value: SeriesValue) {
    ...
    let entry = self.series.entry(key).or_insert_with(|| {
      (0..self.data_len).map(|_| SeriesValue::na()).collect()
    });
  ...
  pub fn plot(&mut self, name: &str, value: SeriesValue) {
    ...
    if let Some(p) = plots.iter_mut().find(|p| p.name == plot_name) {
      ...
      let mut values = (0..self.data_len).map(|_| SeriesValue::na()).collect::<Vec<_>>();
  ...
  pub fn logical_and(&self, a: SeriesValue, b: SeriesValue) -> SeriesValue {
    ...
    SeriesValue::new(if a.value != 0.0 && b.value != 0.0 { 1.0 } else { 0.0 })
  }
