...
impl PineBuiltinTable {
  ...
  pub fn resolve_any(&self, namespace: Option<&str>, name: &str) -> Option<&BuiltinMapping> {
    self.resolve(namespace, name).or_else(|| self.resolve(None, name))
  }
}
...
fn default_mappings() -> Vec<BuiltinMapping> {
  vec![
    ...
    BuiltinMapping {
      ...
      description: "MACD — ta.macd(source, fast, slow, signal) → DIF, DEA, MACD (multi-return)".to_string(),
    },
    BuiltinMapping {
      namespace: Some("ta".to_string()),
    ...
    BuiltinMapping {
      ...
      description: "Bollinger Bands — ta.bb(source, length, mult) → BOLLMID, BOLLUP, BOLLDN".to_string(),
    },
    // --- math.* ---
    BuiltinMapping {
    ...
    BuiltinMapping {
      ...
      description: "Cross-timeframe data — request.security(sym, tf, expr) → SECURITY".to_string(),
    },
    ...
    BuiltinMapping {
      ...
      description: "SuperTrend — ta.supertrend(factor, atrPeriod) → SUPERTREND, DIRECTION".to_string(),
    },
    BuiltinMapping {
      namespace: Some("ta".to_string()),
    ...
    BuiltinMapping {
      ...
      multi_return: true,
      return_names: vec!["ADX".to_string(), "PLUS_DI".to_string(), "MINUS_DI".to_string()],
      description: "Directional Movement Index — ta.dmi(diLength, adxSmoothing) → ADX, +DI, -DI".to_string(),
    },
    BuiltinMapping {
      namespace: Some("ta".to_string()),
    ...
    BuiltinMapping {
      ...
      description: "Volume Weighted Moving Average — ta.vwma(source, length) → VWMA".to_string(),
    },
    BuiltinMapping {
      namespace: Some("ta".to_string()),
