...
use crate::formula::pine::parser::{
  FunctionBody, PineAst, PineAstNode, PineBinaryOp, PineUnaryOp,
};
...
impl<'a> PineAstMapper<'a> {
  ...
  fn map_node(&self, node: &PineAstNode) -> Result<AstNode, PineMapperError> {
    match node {
      ...
      PineAstNode::FunctionDecl { name, params: _, body } => {
  ...
  fn map_tuple_assign(
    ...
    if let PineAstNode::FunctionCall { namespace, name, args } = expr {
      ...
      let expanded: Option<Vec<AstNode>> = match (namespace.as_deref(), name.as_str()) {
        ...
        (Some("ta"), "bb") if mapped_args.len() >= 2 => {
          ...
          Some(vec![
            assignment(&names[0], AstNode::FunctionCall { name: "BOLLMID".to_string(), args: mapped_args.clone() }),
            assignment(&names[1], AstNode::FunctionCall { name: "BOLLUP".to_string(), args: mapped_args.clone() }),
            assignment(&names[2], AstNode::FunctionCall { name: "BOLLDN".to_string(), args: mapped_args.clone() }),
          ])
        }
        (Some("ta"), "dmi") if mapped_args.len() >= 2 => {
          ...
          Some(vec![
            assignment(&names[0], AstNode::FunctionCall { name: "PLUS_DI".to_string(), args: vec![cl.clone(), l1.clone()] }),
            assignment(&names[1], AstNode::FunctionCall { name: "MINUS_DI".to_string(), args: vec![cl.clone(), l1] }),
            assignment(&names[2], AstNode::FunctionCall { name: "ADX".to_string(), args: vec![hi, lo, cl, l2] }),
          ])
        }
        (Some("ta"), "supertrend") if mapped_args.len() >= 2 => {
          ...
          Some(vec![
            assignment(&names[0], st),
            assignment(&names[1], dir),
          ])
        }
        (Some("ta"), "aroon") if !mapped_args.is_empty() => {
          ...
          Some(vec![
            assignment(&names[0], AstNode::FunctionCall { name: "AROON_UP".to_string(), args: vec![cl.clone(), length.clone()] }),
            assignment(&names[1], AstNode::FunctionCall { name: "AROON_DN".to_string(), args: vec![cl, length] }),
          ])
        }
        _ => None,
      };
