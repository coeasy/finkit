...
fn parse_function_decl(pair: Pair<Rule>) -> Result<PineAstNode, PineError> {
    ...
    Ok(PineAstNode::FunctionDecl { name, params, body })
}
...
fn parse_assignment(pair: Pair<Rule>) -> Result<PineAstNode, PineError> {
    ...
    let is_reassign = op == ":=";
    i += 1;
    let expr_pair = parts.into_iter().nth(i).expect("assignment RHS expression");
        ...
        match arg.as_rule() {
            Rule::fill_arg => {
                ...
                if first.as_rule() == Rule::identifier
                    || first.as_rule() == Rule::namespace
                    || first.as_rule() == Rule::arg_name
                {
...
fn parse_named_args(
    pair: Pair<Rule>,
) -> Result<(PineAstNode, Vec<(Option<String>, PineAstNode)>), PineError> {
    ...
    for arg in inner {
        if arg.as_rule() == arg_rule {
            ...
            if item.as_rule() == Rule::identifier
                || item.as_rule() == Rule::namespace
                || item.as_rule() == Rule::arg_name
            {
...
mod tests {
    ...
    fn test_parse_indicator() {
        ...
        assert!(ast
            .items
            .iter()
            .any(|n| matches!(n, PineAstNode::IndicatorDecl { .. })));
    }
    ...
    fn test_parse_ternary() {
        ...
        assert!(ast
            .items
            .iter()
            .any(|n| { matches!(n, PineAstNode::Assignment { .. }) }));
    }
