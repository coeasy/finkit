...
impl Position {
    fn from_signal(s: Signal, allow_short: bool) -> Self {
        if s > 0 { Self::Long } else if s < 0 && allow_short { Self::Short } else { Self::Flat }
    }
}
...
impl Default for BacktestConfig {
    fn default() -> Self {
        Self { initial_cash: 100_000.0, commission: 0.001, slippage: 0.0, allow_short: false }
    }
}
...
pub fn backtest(close: &[f64], signal: &Array1<Signal>, config: &BacktestConfig) -> Result<BacktestResult> {
    let n = close.len();
    if signal.len() != n {
        return Err(TaError::InvalidParameter { name: "close, signal".to_string(), constraint: "must have the same length".to_string() });
    }
    if n < 2 {
        return Err(TaError::InvalidParameter { name: "close".to_string(), constraint: "length must be >= 2".to_string() });
    }

    let mut pos_arr = vec![0i32; n];
    pos_arr[0] = match Position::from_signal(signal[0], config.allow_short) { Position::Long => 1, Position::Short => -1, Position::Flat => 0 };
    for i in 1..n {
        pos_arr[i] = match Position::from_signal(signal[i - 1], config.allow_short) { Position::Long => 1, Position::Short => -1, Position::Flat => 0 };
    }

    let mut bar_returns = one_period_returns(close, ReturnKind::Arithmetic);
    if !bar_returns.is_empty() { bar_returns[0] = 0.0; }
    ...
    for i in 0..n {
        ...
        if new_pos != prev_pos {
            if prev_pos != 0 {
                ...
                trades.push(Trade { entry_idx, exit_idx: i, direction: prev_pos, entry_price, exit_price, pnl, return_pct: pnl / entry_price.max(1e-15) });
            }
            if new_pos != 0 {
                entry_idx = i;
        ...
        if current_pos != 0 { strat_returns[i] += current_pos as f64 * bar_returns[i]; }
    }
    ...
    if current_pos != 0 {
        ...
        trades.push(Trade { entry_idx, exit_idx, direction: current_pos, entry_price, exit_price, pnl, return_pct: pnl / entry_price.max(1e-15) });
    }

    let mut equity = Array1::<f64>::zeros(n);
    equity[0] = config.initial_cash;
    for i in 1..n { equity[i] = equity[i - 1] * (1.0 + strat_returns[i]); }
    ...
    Ok(BacktestResult { total_return, annual_return, sharpe, sortino, max_drawdown: max_dd, win_rate, profit_loss_ratio, n_trades: trades.len(), equity_curve: equity, trades })
}

fn compute_trade_stats(trades: &[Trade]) -> (f64, f64) {
    if trades.is_empty() { return (0.0, 0.0); }
    ...
    let avg_win = if wins.is_empty() { 0.0 } else { wins.iter().map(|trade| trade.pnl).sum::<f64>() / wins.len() as f64 };
    let avg_loss = if losses.is_empty() { 0.0 } else { losses.iter().map(|trade| trade.pnl.abs()).sum::<f64>() / losses.len() as f64 };
    (win_rate, if avg_loss > 1e-15 { avg_win / avg_loss } else { 0.0 })
}
...
mod tests {
    ...
    fn test_backtest_monotonic_up_long() {
        ...
        let r = backtest(&close, &signal, &BacktestConfig { initial_cash: 100_000.0, commission: 0.0, slippage: 0.0, allow_short: false }).unwrap();
    ...
    fn test_backtest_monotonic_down_short() {
        ...
        let signal = Array1::from(vec![-100i32; n]);
        let r = backtest(&close, &signal, &BacktestConfig { initial_cash: 100_000.0, commission: 0.0, slippage: 0.0, allow_short: true }).unwrap();
        assert!(r.total_return > 0.0);
    }
    ...
    fn test_backtest_commission_drag() {
        let n = 10;
        let close: Vec<f64> = (0..n).map(|i| 100.0 + i as f64).collect();
        let signal = Array1::from((0..n).map(|i| if i % 2 == 0 { 100_i32 } else { 0_i32 }).collect::<Vec<_>>());
        let config = BacktestConfig { initial_cash: 100_000.0, commission: 0.01, slippage: 0.0, allow_short: false };
        let r = backtest(&close, &signal, &config).unwrap();
        let r0 = backtest(&close, &signal, &BacktestConfig { commission: 0.0, ..config.clone() }).unwrap();
    ...
    fn validation_errors_are_preserved() {
        assert!(backtest(&[100.0,101.0], &Array1::from(vec![100i32]), &BacktestConfig::default()).is_err());
        assert!(backtest(&[100.0], &Array1::from(vec![100i32]), &BacktestConfig::default()).is_err());
    }
}
