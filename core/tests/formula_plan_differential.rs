...
const DOMESTIC_UNSUPPORTED: &[(&str, &str)] = &[
    ...
    // surface (`functions_talib_081.rs`). They were callable through the tree,
    // bytecode and JIT paths, but no `CALL:<NAME>` kernel existed, so the
    // compiled plan path answered `unsupported kernel` (code 1). The seventeen
    // corpus cases below were the coverage for that gap.
    //
    // 2026-09-24 — the gap is closed. `unified_dispatch.rs` now routes all
    // 44 TA-Lib 0.7/0.8 `CALL:<NAME>` kernels through `dispatch_modern_call`,
    // which delegates each to the exact `canonical_*` implementation the
    // tree/bytecode/JIT paths use. The compiled-plan path therefore emits
    // byte-identical output, so the seventeen cases now pass the differential
    // gate and this list is empty again. The gate still fails loudly if any
    // future change breaks that parity, which is why the list stays in place.
];
