...
fn test_indicator_accuracy(_c: &mut Criterion) {
    ...
    if let Ok(result) = moving_avg::sma(&close, 20) {
        ...
        println!(
            "✓ SMA(20): {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::mama(&close, 0.5, 0.05) {
        ...
        println!(
            "✓ MAMA(0.5,0.05): {} valid values, last={:.4}",
            valid_count, result.mama[999]
        );
    }
    ...
    if let Ok(result) = indicators::macd(&close, 12, 26, 9) {
        ...
        println!(
            "✓ MACD(12,26,9): {} valid values, last={:.4}",
            valid_count, result.macd[999]
        );
    }
    ...
    if let Ok(result) = indicators::aroon(&high, &low, 14) {
        ...
        println!(
            "✓ AROON(14): {} valid values, last_up={:.4}",
            valid_count, result.aroon_up[999]
        );
    }
    ...
    if let Ok(result) = indicators::bbands(&close, 20, 2.0, 2.0) {
        ...
        println!(
            "✓ BBANDS(20,2,2): {} valid values, middle={:.4}",
            valid_count, result.middle[999]
        );
    }
    ...
    if let Ok(result) = indicators::adosc(&high, &low, &close, &volume, 3, 10) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ ADOSC(3,10): {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::ht_dcperiod(&close) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ HT_DCPERIOD: {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::ht_dcphase(&close) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ HT_DCPHASE: {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::ht_trendline(&close) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ HT_TRENDLINE: {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::ht_trendmode(&close) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ HT_TRENDMODE: {} valid values, last={:.0}",
            valid_count, result[999]
        );
    }
    ...
    if let Ok(result) = indicators::ultosc(&high, &low, &close, 7, 14, 28) {
        let valid_count = result.iter().filter(|x| !x.is_nan()).count();
        println!(
            "✓ ULTOSC(7,14,28): {} valid values, last={:.4}",
            valid_count, result[999]
        );
    }
