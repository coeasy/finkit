...
The Node binding has a source-build path and a real `node:test` smoke suite. The multi-language workflow defines the CI packaging gate: it must build the native module, run the smoke suite, stage the platform native file, and validate `npm pack` for each published target.

The `v0.1.15` release workflow builds Node packages, but this documentation does not assume that the root `finkit` package or all optional native platform packages have been published to npm.
...
Do not use `npm install finkit` as a guaranteed public installation instruction until the npm registry and all required native dependency packages have been verified. For v0.1.15, the release artifact or source build is the documented Node path.
