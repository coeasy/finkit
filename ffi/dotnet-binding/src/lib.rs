...
pub unsafe extern "C" fn ta_formula_eval_temporal_contract_json(
    ...
    ffi_catch_ptr(|| {
        ...
        let Some(request) = read_c_string(request) else {
            return error("formula temporal contract request is not valid UTF-8");
        };
        let payload =
            finkit_ffi_common::evaluate_formula_temporal_json(&request).unwrap_or_else(|message| {
                serde_json::json!({
                    "schema_version": finkit_ffi_common::FORMULA_TEMPORAL_CONTRACT_SCHEMA_VERSION,
                    "error": message,
                })
...
pub unsafe extern "C" fn ta_formula_eval_panel_contract_json(
    ...
    ffi_catch_ptr(|| {
        ...
        let Some(request) = read_c_string(request) else {
            return error("formula panel contract request is not valid UTF-8");
        };
        let payload =
...
pub unsafe extern "C" fn ta_formula_stream_execute_json(request: *const c_char) -> *mut c_char {
    ffi_catch_ptr(|| {
        ...
        CString::new(payload)
            .expect("formula stream result payload contains no NUL")
            .into_raw()
    })
}
...
pub unsafe extern "C" fn ta_factor_stream_execute_json(request: *const c_char) -> *mut c_char {
    ffi_catch_ptr(|| {
        ...
        CString::new(payload)
            .expect("factor stream result payload contains no NUL")
            .into_raw()
    })
}
...
pub unsafe extern "C" fn ta_composite_execute_json(request: *const c_char) -> *mut c_char {
    ffi_catch_ptr(|| {
        ...
        CString::new(payload)
            .expect("composite result payload contains no NUL")
            .into_raw()
    })
}
...
pub unsafe extern "C" fn ta_composite_stream_execute_json(request: *const c_char) -> *mut c_char {
    ffi_catch_ptr(|| {
        ...
        CString::new(payload)
            .expect("composite stream result payload contains no NUL")
            .into_raw()
    })
}
...
pub extern "C" fn ta_formula_eval_multi(
    ...
    ffi_catch_ptr(|| {
        ...
        let source_str = match unsafe { CStr::from_ptr(source) }
            .to_str()
            .map(str::to_owned)
        {
            Ok(s) => s,
...
pub extern "C" fn ta_formula_search_templates(keyword: *const c_char) -> *mut c_char {
    ffi_catch_ptr(|| {
        ...
        let keyword_str = match unsafe { CStr::from_ptr(keyword) }
            .to_str()
            .map(str::to_owned)
        {
            Ok(s) => s,
