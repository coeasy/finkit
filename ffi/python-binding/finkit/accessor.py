"""Pandas DataFrame accessor for finkit technical analysis.
...
class TaAccessor:
    ...
    def _call_indicator(self, name: str, *args: Any, **kwargs: Any) -> Any:
        try:
            import finkit as _ta
        except ImportError:
            from finkit import finkit as _ta

        fn = getattr(_ta, name, None)
        if fn is None:
            raise AttributeError(f"finkit has no indicator '{name}'")
