...
func Version() string { return C.GoString(C.ta_version()) }

func convertResult(result *C.TaResult) ([]float64, error) {
	defer C.ta_free_result(result)
	if result.error != nil { return nil, errors.New(C.GoString(result.error)) }
	if result.data == nil || result.length == 0 { return nil, nil }
...
func cInt(n int) C.int { return C.int(n) }
func cDouble(f float64) C.double { return C.double(f) }

func Sma(input []float64, period int) ([]float64, error) { return convertResult(C.ta_sma(toCSlice(input), cInt(len(input)), cInt(period))) }
func Ema(input []float64, period int) ([]float64, error) { return convertResult(C.ta_ema(toCSlice(input), cInt(len(input)), cInt(period))) }
func Wma(input []float64, period int) ([]float64, error) { return convertResult(C.ta_wma(toCSlice(input), cInt(len(input)), cInt(period))) }
func Dema(input []float64, period int) ([]float64, error) { return convertResult(C.ta_dema(toCSlice(input), cInt(len(input)), cInt(period))) }
func Tema(input []float64, period int) ([]float64, error) { return convertResult(C.ta_tema(toCSlice(input), cInt(len(input)), cInt(period))) }
func Kama(input []float64, period int) ([]float64, error) { return convertResult(C.ta_kama(toCSlice(input), cInt(len(input)), cInt(period))) }
func T3(input []float64, period int, vfactor float64) ([]float64, error) { return convertResult(C.ta_t3(toCSlice(input), cInt(len(input)), cInt(period), cDouble(vfactor))) }
func Rsi(input []float64, period int) ([]float64, error) { return convertResult(C.ta_rsi(toCSlice(input), cInt(len(input)), cInt(period))) }

func Macd(input []float64, fastPeriod, slowPeriod, signalPeriod int) (*MacdResult, error) {
	arrays, err := convertMultiResult(C.ta_macd(toCSlice(input), cInt(len(input)), cInt(fastPeriod), cInt(slowPeriod), cInt(signalPeriod)), 3)
...
func Stoch(high, low, close []float64, kPeriod, kSlow, dPeriod int) (*StochResult, error) {
	...
	arrays, err := convertMultiResult(C.ta_stoch(toCSlice(high), toCSlice(low), toCSlice(close), cInt(length), cInt(kPeriod), cInt(kSlow), cInt(dPeriod)), 2)
	if err != nil { return nil, err }
...
func Aroon(high, low []float64, period int) (*AroonResult, error) {
	length := len(high); if length != len(low) { return nil, errors.New("high and low must have the same length") }
...
func Bbands(input []float64, period int, nbDevUp, nbDevDn float64) (*BbandsResult, error) {
	...
	if err != nil { return nil, err }; if len(arrays) != 3 { return nil, errors.New("unexpected Bollinger Bands result format") }
...
func HtPhasor(input []float64) (*HtPhasorResult, error) {
	arrays, err := convertMultiResult(C.ta_ht_phasor(toCSlice(input), cInt(len(input))), 2); if err != nil { return nil, err }; if len(arrays) != 2 { return nil, errors.New("unexpected Hilbert Phasor result format") }; return &HtPhasorResult{InPhase: arrays[0], Quadrature: arrays[1]}, nil
}
func HtSine(input []float64) (*HtSineResult, error) {
	arrays, err := convertMultiResult(C.ta_ht_sine(toCSlice(input), cInt(len(input))), 2); if err != nil { return nil, err }; if len(arrays) != 2 { return nil, errors.New("unexpected Hilbert Sine result format") }; return &HtSineResult{Sine: arrays[0], LeadSine: arrays[1]}, nil
}
...
func Beta(asset, benchmark []float64, period int) ([]float64, error) {
	length := len(asset); if length != len(benchmark) { return nil, errors.New("asset and benchmark must have the same length") }; return convertResult(C.ta_beta(toCSlice(asset), toCSlice(benchmark), cInt(length), cInt(period)))
}
func Correlation(inputA, inputB []float64, period int) ([]float64, error) {
	length := len(inputA); if length != len(inputB) { return nil, errors.New("inputA and inputB must have the same length") }; return convertResult(C.ta_correlation(toCSlice(inputA), toCSlice(inputB), cInt(length), cInt(period)))
}
...
func FormulaEval(source string, open, high, low, close, volume []float64) (map[string][]float64, error) {
	...
	for name, values := range result { arr := make([]float64, len(values)); for i, v := range values { if v != nil { arr[i] = *v } }; out[name] = arr }
	return out, nil
}
...
func formulaJSONResult(result *C.char) (string, error) { if result == nil { return "", errors.New("native formula call returned a null result") }; defer C.ta_free_string(result); value := C.GoString(result); if !json.Valid([]byte(value)) { return "", errors.New(value) }; return value, nil }
func FormulaEvalMultiJSON(source string, open, high, low, close, volume []float64) (string, error) { length:=len(open); if len(high)!=length||len(low)!=length||len(close)!=length||len(volume)!=length{return "",errors.New("all input arrays must have the same length")}; cSource:=C.CString(source); defer C.free(unsafe.Pointer(cSource)); return formulaJSONResult(C.ta_formula_eval_multi(cSource,toCSlice(open),toCSlice(high),toCSlice(low),toCSlice(close),toCSlice(volume),cInt(length))) }
func FormulaEvalDrawJSON(source string, open, high, low, close, volume []float64) (string, error) { length:=len(open); if len(high)!=length||len(low)!=length||len(close)!=length||len(volume)!=length{return "",errors.New("all input arrays must have the same length")}; cSource:=C.CString(source); defer C.free(unsafe.Pointer(cSource)); return formulaJSONResult(C.ta_formula_eval_draw(cSource,toCSlice(open),toCSlice(high),toCSlice(low),toCSlice(close),toCSlice(volume),cInt(length))) }
func FormulaEvalDebugJSON(source string, open, high, low, close, volume []float64) (string, error) { length:=len(open); if len(high)!=length||len(low)!=length||len(close)!=length||len(volume)!=length{return "",errors.New("all input arrays must have the same length")}; cSource:=C.CString(source); defer C.free(unsafe.Pointer(cSource)); return formulaJSONResult(C.ta_formula_eval_debug(cSource,toCSlice(open),toCSlice(high),toCSlice(low),toCSlice(close),toCSlice(volume),cInt(length))) }
...
func DarvasBoxJSON(high,low,close []float64,lookback,confirmation int)(string,error){if len(high)!=len(low)||len(high)!=len(close){return "",errors.New("all input arrays must have the same length")};return formulaJSONResult(C.ta_darvas_box_json(toCSlice(high),toCSlice(low),toCSlice(close),cInt(len(high)),cInt(lookback),cInt(confirmation)))}
func RenkoJSON(high,low []float64,boxSize float64)(string,error){if len(high)!=len(low){return "",errors.New("all input arrays must have the same length")};return formulaJSONResult(C.ta_renko_json(toCSlice(high),toCSlice(low),cInt(len(high)),C.double(boxSize)))}
...
func PointAndFigureJSON(high,low []float64,boxSize float64,reversal int)(string,error){if len(high)!=len(low){return "",errors.New("all input arrays must have the same length")};return formulaJSONResult(C.ta_point_and_figure_json(toCSlice(high),toCSlice(low),cInt(len(high)),C.double(boxSize),cInt(reversal)))}
...
func HeikinAshiJSON(open,high,low,close []float64)(string,error){if len(open)!=len(high)||len(open)!=len(low)||len(open)!=len(close){return "",errors.New("all input arrays must have the same length")};return formulaJSONResult(C.ta_heikin_ashi_json(toCSlice(open),toCSlice(high),toCSlice(low),toCSlice(close),cInt(len(open))))}
func FormulaEvalZeroCopy(source string,open,high,low,close,volume []float64)(map[string][]float64,error){length:=len(open);if len(high)!=length||len(low)!=length||len(close)!=length||len(volume)!=length{return nil,errors.New("all input arrays must have the same length")};cSource:=C.CString(source);defer C.free(unsafe.Pointer(cSource));cResult:=C.ta_formula_eval_zc_exec(cSource,toCSlice(open),toCSlice(high),toCSlice(low),toCSlice(close),toCSlice(volume),cInt(length));defer C.ta_free_string(cResult);resultStr:=C.GoString(cResult);if len(resultStr)>6&&resultStr[:6]=="error:"{return nil,errors.New(resultStr[7:])};var result map[string][]*float64;if err:=json.Unmarshal([]byte(resultStr),&result);err!=nil{return nil,err};out:=make(map[string][]float64);for name,values:=range result{arr:=make([]float64,len(values));for i,v:=range values{if v!=nil{arr[i]=*v}};out[name]=arr};return out,nil}
...
type MacdOutput struct{Macd,Signal,Hist float64}
type StreamingMacd struct{handle unsafe.Pointer}
type MaType int
const(MaSMA MaType=iota;MaEMA;MaWMA;MaDEMA;MaTEMA;MaKAMA;MaT3;MaTRIMA;MaHMA;MaALMA;MaVIDYA)
type StreamingMacdExt struct{handle unsafe.Pointer}
func NewStreamingMacdExt(fastPeriod int,fastMa MaType,slowPeriod int,slowMa MaType,signalPeriod int,signalMa MaType)*StreamingMacdExt{h:=C.ta_streaming_macd_ext_new(cInt(fastPeriod),cInt(int(fastMa)),cInt(slowPeriod),cInt(int(slowMa)),cInt(signalPeriod),cInt(int(signalMa)));if h==nil{return nil};return &StreamingMacdExt{handle:unsafe.Pointer(h)}}
func(s *StreamingMacdExt)Update(value float64)(MacdOutput,bool){if s==nil||s.handle==nil{return MacdOutput{},false};var macd,sig,hist C.double;ready:=C.ta_streaming_macd_ext_update(s.handle,cDouble(value),&macd,&sig,&hist);if ready==0{return MacdOutput{},false};return MacdOutput{Macd:float64(macd),Signal:float64(sig),Hist:float64(hist)},true}
...
type BbandsOutput struct{Upper,Middle,Lower float64}
