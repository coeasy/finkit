...
extern char* ta_factor_cross_sectional_execute_json(const char *request_json);
...
// FactorCrossSectionalExecuteJSON evaluates one cross-sectional Factor over
// row-major symbol panels using the shared contract.
func FactorCrossSectionalExecuteJSON(requestJSON string) (string, error) {
	cRequest := C.CString(requestJSON)
	defer C.free(unsafe.Pointer(cRequest))
	return formulaJSONResult(C.ta_factor_cross_sectional_execute_json(cRequest))
}

// FactorStreamExecuteJSON appends rows through the shared bounded streaming
// contract and returns a portable checkpoint for the next call.
func FactorStreamExecuteJSON(requestJSON string) (string, error) {
	cRequest := C.CString(requestJSON)
	defer C.free(unsafe.Pointer(cRequest))
	return formulaJSONResult(C.ta_factor_stream_execute_json(cRequest))
}

// CompositeStreamExecuteJSON appends rows through the shared bounded streaming
// contract and returns a portable checkpoint for the next call.
func CompositeStreamExecuteJSON(requestJSON string) (string, error) {
	cRequest := C.CString(requestJSON)
	defer C.free(unsafe.Pointer(cRequest))
	return formulaJSONResult(C.ta_composite_stream_execute_json(cRequest))
}
...
// FormulaEvalTemporalContractJSON evaluates an explicit timestamped Formula
// request using the shared multi-timeframe and point-in-time contract.
func FormulaEvalTemporalContractJSON(requestJSON string) (string, error) {
	cRequest := C.CString(requestJSON)
	defer C.free(unsafe.Pointer(cRequest))
