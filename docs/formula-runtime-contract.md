...
### 3.1 Warm-up 可组合性（预热段不得污染下游）

滚动类指标（`MA`/`EMA`/`WMA`/`RSI`/`STD`/`BOLL`/`ZSCORE`/`LINEARREG*` 等）的输出以一段**前导 NaN 预热段**开头，这是约定语义，不是错误。

**契约**：把这样的输出继续作为输入参与计算时，预热段**不得**使下游结果整段变成 NaN。也就是说下列写法必须产生与"先手工对齐再计算"一致的有效值：
...
AVGDEV(MA(CLOSE,5),9)
ZSCORE(MA(CLOSE,5),9)
LINEARREG(MA(CLOSE,5),9)
```

实现约束（对所有含增量累加器 `sum += newest - oldest` 的核）：

- 递推的种子窗口必须从**第一个有限值**开始，而不是固定从下标 `0` 开始；
- 除了种子与循环边界，**每一个内部下标判断（例如折进主循环的滑窗 guard）也必须按该起点偏移**；
- 有限尾巴短于窗口时，保持全 NaN（与既有 `window > input.len()` 语义一致）；
- **输入完全有限时行为必须逐位不变**，因此该保证不改变既有 golden / benchmark 结果。

**内部**非有限值（预热段之后的 NaN/Inf）仍按既有约定使结果变为 NaN；本节只覆盖**前导**预热段。

### 3.2 多条执行路径必须数值一致

同一个公式存在多条执行路径：tree（参考解释器）、bytecode、编译计划（plan kernel），以及在启用对应 feature 时的 JIT / SIMD。

**契约**：这些路径对同一输入必须给出相同结果。门禁要求"一致**且**有有限值"——只比较两路径是否相等是不够的，因为两边一致地返回全 NaN 也会被判为"一致"。因此：

- 每个数值门禁同时断言**绝对性质**（例如精确的有限值个数），而不只断言路径间相等；
...
- 退化输入（周期大于序列长度、只有 1 根数据）在所有路径上都产出**等长全 NaN 序列**而非错误；同时断言周期为 0 在**所有**路径上仍被拒绝；
