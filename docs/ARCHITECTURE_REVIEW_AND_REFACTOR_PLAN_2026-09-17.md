...
- `operation_execute_json` 已支持显式 `semantic_profile=core_registry` 与 `semantic_profile=talib_0_7_1`；后者目前覆盖 SMA/EMA/WMA、RSI、MACD、BBANDS、ATR/NATR/TRANGE、ADX、CCI、STOCH、WILLR、MOM、ROC、OBV、MFI，其余 TA-Lib catalog 仍返回结构化 `unsupported_operation`。TA-Lib 与国内终端同名函数必须继续在 variant/profile registry 中显式区分，禁止把同名 `SMA` 等语义静默合并。
