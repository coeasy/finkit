...
- Formula 已新增 `formula.stream.contract.v1`：以统一 JSON 请求执行可序列化的 `MA/SMA/WMA/EMA/RSI/ATR/MACD/HHV/LLV/SUM/STD/STDDEV/VAR/REF/CROSS/CROSSBELOW` 状态内核，并支持单表达式及数值赋值/多语句程序中的算术、比较、逻辑、`IF`、一元数学函数组合，以及常量边界、有迭代上限的 `FOR` 数值程序；返回 `__PRIMARY__`、null warm-up、rows、required inputs、稳定 signature 和对象形式 checkpoint。Rust、Python、Go、Java、.NET、C、C++、Node 均已补齐同一入口，并加入 Formula/Factor/Composite 共用的续算 conformance vector。`STD/VAR` 使用 Formula/TA-Lib 约定的总体方差语义，未复用样本方差的公共指标状态。该入口仍明确拒绝动态/无界循环、`WHILE`、绘图、跨周期、未来数据和未知变量等尚未具备可移植状态布局的语义，不把批量解释执行冒充 stateful streaming。
