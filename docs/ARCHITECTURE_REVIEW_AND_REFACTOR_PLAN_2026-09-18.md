...
- Formula 已新增 `formula.stream.contract.v1`：以统一 JSON 请求执行可序列化的直接 `MA/SMA/WMA/EMA/RSI/ATR/MACD/HHV/LLV/SUM/STD/STDDEV/VAR/REF/CROSS/CROSSBELOW` 状态内核，返回 `__PRIMARY__`、null warm-up、rows、required inputs、稳定 signature 和对象形式 checkpoint；Rust、Python、Go、Java、.NET、C、C++、Node 均已补齐同一入口，并加入 Formula/Factor/Composite 共用的续算 conformance vector。`STD/VAR` 使用 Formula/TA-Lib 约定的总体方差语义，未复用样本方差的公共指标状态。该入口明确拒绝赋值、多语句、控制流、绘图、跨周期和未知变量等尚未具备可移植状态布局的语义，不把批量解释执行冒充 stateful streaming。
...
- `cargo +1.98.1 test --workspace --offline --quiet`：全 workspace 测试通过；其中核心库为 `2940 passed, 0 failed, 1 ignored`，Formula/Factor/Composite 共用 contract conformance 为 `1 passed, 0 failed`，DZH compatibility 为 `43 passed, 0 failed`，CLI schema 为 `3 passed, 0 failed`，其余 workspace test targets 也无失败。
...
- Formula stateful stream 已覆盖直接 MA/SMA/WMA/EMA/RSI/ATR/MACD/HHV/LLV/SUM/STD/STDDEV/VAR/REF/CROSS/CROSSBELOW 子集，但不代表完整 Formula 赋值、控制流、绘图、跨周期或 Pine/国内终端语义已经拥有可移植状态执行器；
