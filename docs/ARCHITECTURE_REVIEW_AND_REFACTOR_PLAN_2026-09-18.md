...
- 新增 161 个 TA-Lib dispatcher 名称的 JSON smoke test：逐项经过统一请求、分派和结果 envelope，确认返回结构及等长输出；这属于执行链覆盖验证，不等同于 161 项数值等价验证。矩阵同时明确 101 个 profile-only 注册项和 161 个固定 numeric golden 项。
