...
- 新增 161 个 TA-Lib dispatcher 名称的 JSON smoke test：逐项经过统一请求、分派和结果 envelope，确认返回结构及等长输出；这属于执行链覆盖验证，不等同于 161 项数值等价验证。矩阵同时明确 101 个 profile-only 注册项和 89 个固定 numeric golden 项。
...
- TA-Lib golden：定向全指标 suite `1 passed, 0 failed`，89 个指标均有固定 reference 文件；当前集合不是 TA-Lib 全目录证明。
- TA-Lib coverage matrix：profile-only catalog `101`、dispatcher smoke `161`、fixed numeric golden `89`；矩阵与 golden 文件、公共 catalog、dispatcher 支持集合已通过一致性测试。
