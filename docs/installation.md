...
This guide distinguishes the installation paths actually published in Finkit **v0.1.12** from the additional multi-language package candidates being validated for the next release.

## 1. Choose an installation path

| Target | Recommended path today | Published v0.1.12 status | Next-release validation |
| --- | --- | --- | --- |
| Python | Install a wheel from the GitHub `v0.1.12` Release | Verified Release wheels | existing wheel matrix retained |
...
| Go/CGO | Build from `ffi/go-binding` | source only in v0.1.12 | Linux `go test` + external module gate |
| .NET | Build from `ffi/dotnet-binding` | source only in v0.1.12 | Linux tests + NuGet candidate inspection |
| Android | Build Rust JNI + AAR | source only in v0.1.12 | four-ABI AAR gate |
| iOS | Build XCFramework from source | source only in v0.1.12 | device/simulator XCFramework gate |
| WASM | Build `finkit-wasm` for wasm32 | source only in v0.1.12 | real wasm32 target gate |

The published `v0.1.12` GitHub Release is:

`https://github.com/coeasy/finkit/releases/tag/v0.1.12`

Its assets are four Python ABI3 wheels, `finkit-0.1.12.crate`, `finkit-cli-linux-x86_64`, and `SHA256SUMS`.

A next-release CI artifact is not automatically a v0.1.12 asset, a final future Release asset, or a public registry package.
...
To reproduce the published v0.1.12 source contract:
...
### Supported v0.1.12 wheel matrix

The `v0.1.12` Release uses CPython stable ABI (`cp38-abi3`). One wheel per platform is reused by supported GIL-enabled CPython versions.

| Platform | Release wheel family | CI compatibility |
| --- | --- | --- |
| Linux x86_64 | `finkit-0.1.12-cp38-abi3-manylinux_2_17_x86_64...whl` | CPython 3.8-3.14 |
| Windows x86_64 | `finkit-0.1.12-cp38-abi3-win_amd64.whl` | build/install smoke verified |
| macOS x86_64 | `finkit-0.1.12-cp38-abi3-macosx_*_x86_64.whl` | build/install smoke verified |
| macOS arm64 | `finkit-0.1.12-cp38-abi3-macosx_*_arm64.whl` | build/install smoke verified |

Not part of the v0.1.12 wheel matrix: Linux arm64, musllinux, 32-bit Windows, PyPy, and free-threaded CPython.

### Install a Release wheel

```bash
python -m pip install --upgrade pip
python -m pip install ./finkit-0.1.12-<matching-platform>.whl
```
...
The workspace package is named `finkit`. The repository produces `finkit-0.1.12.crate` as a Release asset, but that does not imply crates.io publication.

Git tag dependency:

```toml
[dependencies]
finkit = { git = "https://github.com/coeasy/finkit", tag = "v0.1.12" }
```
...
Download the v0.1.12 Linux x86_64 binary:

```bash
curl -L -o finkit-cli \
  https://github.com/coeasy/finkit/releases/download/v0.1.12/finkit-cli-linux-x86_64
chmod +x finkit-cli
./finkit-cli --help
```
...
Do not assume `com.finkit:finkit:0.1.12` is downloadable from Maven Central until publication is independently verified.
