...
5 分钟上手 Finkit（finkit）。详细安装见 [installation.md](./installation.md)。
...
| Python | `finkit`（PyO3）| `pip install finkit` 或 `cd ffi/python-binding && maturin develop` |
| Node | `finkit`（napi-rs）| `npm install finkit` |
| Go | `github.com/coeasy/finkit/go/ta` | `go get github.com/coeasy/finkit/go/ta` |
| Java | `com.finkit`（JNI）| 编译 `.jar` 后引用 |
| .NET | `Finkit`（P/Invoke）| NuGet 包 |
| C/C++ | `finkit-ffi` + `finkit.h` | 链接动态库 |
| CLI | `finkit` | `cargo install --path cli` |
...
use finkit::streaming::{indicators::StreamingRsi, StreamingIndicator};
...
### CLI
```bash
finkit sma -i data.csv --period 14
finkit macd -i data.csv --fast 12 --slow 26 --signal 9
