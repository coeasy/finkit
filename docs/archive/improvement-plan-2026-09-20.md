...
## 17. 新发现的缺口类别：绘图原语（`DRAW:*`）—— 已裁决（2026-09-20 续）
...
- 剩余 Pine 条目 **3 条**：`trix`（待裁决实现分歧 → **已在 §18 解决**）、
  `supertrend`（`IF` 无降级）、`volume_profile`（循环体不降级）。
...
## 18. `TRIX`：最后一次实现分歧裁决（2026-09-20 续）

### 18.1 为什么它不是"补个 kernel"就行

`trix` 是白名单里**唯一被数值分歧挡住**的条目（其余两条是结构性降级缺口）。
两处实现都在，但**算出来的数不一样**：

| | 实现 | 种子做法 |
|---|---|---|
| 树路径 | `fn_trix`（手写，三串 `lib_ma::ema`） | 把预热 NaN **替换成 `0.0`** 再喂下一层 |
| 指标层 | `trix_into` | 每层 EMA 用**自己输入的 SMA** 正规种子，预热留 NaN |

§15.1 的规则是"有 golden 覆盖的一方赢，手写的一方收敛"。但 §17 之后这条规则
多了个前提：**golden 必须覆盖完整契约**（SAR 就是反例：`overlap::sar` 有 golden，
但只吃 `(acceleration, maximum)`，表达不了 Pine 的三参数）。

先查证：`trix_into` **有 TA-Lib golden**（`tests/golden/talib/trix.json`、
`tests/golden/trix_14.csv`，由 `golden_talib_tests.rs` 驱动），且契约就是
`(input, period)` —— 与 `fn_trix` **完全一致**。→ 前提成立，可以照 §15.1 收敛。

### 18.2 先量化，再动手（这是本次与 WILLR 那次的关键差别）

WILLR 那次我是**先改后看**；这次先写了个一次性探针，把两条实现放在同一份输入上
逐点比对（探针对照完即删）。结果说明分歧**不是精度差异，是两处独立的错误**：

| period | 树路径首个有限值 | golden 首个有限值 | 多出来的点 | 最大绝对差 | 尾部残差 |
|---|---|---|---|---|---|
| 5 | 5 | 13 | 8 | 3.751 | 0 |
| 12 | 12 | 34 | 22 | 1.275 | 3.5e-6 |
| 14 | 14 | 40 | 26 | 1.150 | 4.2e-5 |
| 15 | 15 | 43 | 28 | 1.113 | 1.1e-4 |

两种错误：

1. **预热区凭空造值** —— TA-Lib 要求 `3*(period-1)` 才出第一个值，树路径从
   `period` 就出。period=12 时多出 **22 个点**，在 100 根 K 线上是**三分之一条曲线**，
   而且这些点数值不小（如 period=12、第 24 根：树路径 `6.43`，golden 是 NaN）。
2. **零点污染不会消失** —— 喂进 EMA2/EMA3 的 `0.0` 只按几何级数衰减
   （`k = 2/(period+1)`），period=15 时到第 120 根**仍有 1e-4 的残差**。
   也就是说"跑久一点就准了"是**错的**。

> 这个表也说明了为什么不能"取个折中"：预热区的差异是**有无**，不是多少。

### 18.3 处理

- `fn_trix` 删掉手写三串 EMA，改为委托 `momentum::trix`（→ `trix_into`）。
- 新增 `CALL:TRIX` kernel：形状是"单序列 + 周期"，与 `TRIMA`/`RSI` 同类，
  **直接并入 `dispatch_periodic_call`**，不新开一个 handler。
  它调用的正是 `fn_trix` 委托的那个函数 → 两路径**构造性一致**。
- `registry.rs` 补 `TRIX` 规格（`LookbackSpec::Dynamic`，因为预热是
  `3*(period-1)`，`PeriodMinusOne`/`Period` 都表达不了）。

**改完复测探针：period 5/12/14/15 全部逐位相等（最大绝对差 `0.0`）**，
首个有限值下标与有限值个数也完全对齐。

### 18.4 结果

- Pine 语料 **22 → 23**；剩余 **2 条**（`supertrend`、`volume_profile`），**全是结构性降级缺口**。
- 三面规模 **`(225, 416, 58)` → `(226, 416, 59)`**。
- 全量 `cargo test -p finkit`：**4020 passed / 0 failed**（43 个测试二进制），与基线一致
  → 确认没有别的测试依赖旧的错误数值。

### 18.5 教训

> **"跑久了就收敛"是数值 bug 最常见的伪装。**
> 如果只看尾部，两版 TRIX 的差是 1e-4，很容易被当成"精度差异"放过。
> 真正致命的是**预热区多出来的那 22 个点**——那是**有没有值**的问题。
> 所以判分歧时**必须同时看两个量**：`首个有限值下标` 和 `最大绝对差`，
> 只看其中一个都会被骗。
