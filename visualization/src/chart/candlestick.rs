...
pub fn render_candlestick(
    ...
    for i in 0..n {
        ...
        let body_x = plot_area.x + i as f64 * bar_width + gap;
...
mod tests {
    ...
    #[test]
    fn test_candlestick_draw_list_not_empty() {
        let data = make_test_data();
        let config = ChartConfigBuilder::new().build();
        let layout = make_layout(&data, &config);
        let mut draw_list = DrawList::new();
        render_candlestick(&mut draw_list, &data, &layout, &config);
        assert!(!draw_list.is_empty());
    }

    #[test]
    fn test_candlestick_primitives_per_bar() {
        let data = make_test_data();
        let config = ChartConfigBuilder::new().build();
        let layout = make_layout(&data, &config);
        let mut draw_list = DrawList::new();
        render_candlestick(&mut draw_list, &data, &layout, &config);
        assert_eq!(draw_list.len(), 6);
    }
