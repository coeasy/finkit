...
- Rust package: `finkit-0.1.9.crate` Release asset.
...
The GitHub `v0.1.9` Release is the authoritative distribution for that version. It contains:
...
These are **validation targets**, not retroactive v0.1.9 Release assets. A language moves from “source exists” to “CI validated” only when its final-head job is green, and it becomes a published distribution only after the resulting artifact is released and a clean consumer install succeeds.
...
Download the wheel matching your platform from the `v0.1.9` GitHub Release, then install it locally:

```bash
python -m pip install --upgrade pip
python -m pip install ./finkit-0.1.9-<platform>.whl
```
