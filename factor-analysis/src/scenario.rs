...
pub fn volatility_stress(
    covariance: &[Vec<f64>],
    multiplier: f64,
) -> ResearchResult<Vec<Vec<f64>>> {
    if multiplier < 0.0 || !multiplier.is_finite() {
        return Err(ResearchError::InvalidConfig(
            "volatility multiplier must be finite and >= 0".to_string(),
        ));
    }
...
pub fn correlation_stress(
    covariance: &[Vec<f64>],
    target_correlation: f64,
) -> ResearchResult<Vec<Vec<f64>>> {
    if !(-1.0..=1.0).contains(&target_correlation)
        || covariance.iter().any(|row| row.len() != covariance.len())
    {
        return Err(ResearchError::InvalidConfig(
            "invalid correlation stress".to_string(),
        ));
    }
    ...
    for i in 0..n {
        for j in 0..n {
            if i != j {
                out[i][j] = target_correlation * stds[i] * stds[j];
            }
        }
    }
    Ok(out)
