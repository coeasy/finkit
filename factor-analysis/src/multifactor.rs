...
pub fn factor_correlation_matrix(
    frame: &ResearchFrame,
    factors: &[&str],
) -> ResearchResult<Vec<Vec<f64>>> {
    let columns: Vec<&[f64]> = factors
        .iter()
...
pub fn variance_inflation_factors(
    ...
    for i in 0..columns.len() {
        let x: Vec<&[f64]> = columns
            .iter()
            .enumerate()
            .filter_map(|(j, col)| (j != i).then_some(*col))
            .collect();
        if x.is_empty() {
            out.push(1.0);
            continue;
        ...
        out.push(if fit.r_squared >= 1.0 - 1e-12 {
            f64::INFINITY
        } else {
            1.0 / (1.0 - fit.r_squared)
        });
    }
    Ok(out)
}
...
pub fn neutralize_cross_sectional(
    ...
    if exposures.is_empty() {
        return Ok(frame.column(target)?.to_vec());
    }
    let y = frame.column(target)?;
    let x: Vec<&[f64]> = exposures
        .iter()
        .map(|name| frame.column(name))
        .collect::<ResearchResult<_>>()?;
    ...
    for segment in 0..frame.index().date_segments().len() {
        ...
        let local_x_owned: Vec<Vec<f64>> = x
            .iter()
        ...
        if let Ok(fit) = ols(&local_y, &local_x) {
            for (&row, residual) in rows.iter().zip(fit.residuals) {
                out[row] = residual;
            }
        }
    }
...
pub fn pca_project(
    ...
    if factors.is_empty() || components == 0 {
        return Err(ResearchError::InvalidConfig(
            "PCA requires factors and components".to_string(),
        ));
    }
    let columns: Vec<&[f64]> = factors
        .iter()
        .map(|name| frame.column(name))
        .collect::<ResearchResult<_>>()?;
    let rows: Vec<Vec<f64>> = (0..frame.index().len())
    ...
    {
        return Err(ResearchError::InvalidConfig(
            "PCA input must be finite; clean the research frame first".to_string(),
        ));
    }
...
pub fn fama_macbeth(
    ...
    if factors.is_empty() {
        return Err(ResearchError::InvalidConfig(
            "Fama-MacBeth requires at least one factor".to_string(),
        ));
    }
    let y = frame.column(return_column)?;
    let x: Vec<&[f64]> = factors
        .iter()
        .map(|name| frame.column(name))
        .collect::<ResearchResult<_>>()?;
    ...
    for segment in 0..frame.index().date_segments().len() {
        ...
        let local_x: Vec<&[f64]> = local_x_owned.iter().map(Vec::as_slice).collect();
        if let Ok(fit) = ols(&local_y, &local_x) {
            if fit.coefficients.len() == factors.len() {
                premia.push(fit.coefficients);
            }
        }
    }
    if premia.is_empty() {
        return Ok(FamaMacBethResult {
            mean_premia: vec![f64::NAN; factors.len()],
            standard_errors: vec![f64::NAN; factors.len()],
            t_stats: vec![f64::NAN; factors.len()],
            valid_dates: 0,
        });
    }
    ...
    let t_stats = means
        .iter()
        .zip(&ses)
        .map(|(&mean, &se)| if se > 1e-15 { mean / se } else { f64::NAN })
        .collect();
    Ok(FamaMacBethResult {
        mean_premia: means,
        standard_errors: ses,
        t_stats,
        valid_dates: premia.len(),
    })
}
