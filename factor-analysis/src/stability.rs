...
pub fn rolling_mean(values: &[f64], window: usize) -> Vec<f64> {
    ...
    for i in window.saturating_sub(1)..values.len() {
        ...
        let valid: Vec<f64> = values[start..=i]
            .iter()
            .copied()
...
pub fn factor_health(
    ...
    let mean_ic = if valid_ic.is_empty() {
        f64::NAN
    } else {
        valid_ic.iter().sum::<f64>() / valid_ic.len() as f64
    };
    let recent: Vec<f64> = ic
        .iter()
        .rev()
        .copied()
        .filter(|v| v.is_finite())
        .take(recent_window)
        .collect();
    let recent_ic = if recent.is_empty() {
        f64::NAN
    } else {
        recent.iter().sum::<f64>() / recent.len() as f64
    };
    let drift = factor_psi(baseline_factor, current_factor, 10);
    let coverage = if current_factor.is_empty() {
        0.0
    } else {
        current_factor.iter().filter(|v| v.is_finite()).count() as f64 / current_factor.len() as f64
    };
    let degraded = drift.is_finite() && drift > psi_threshold
        || (recent_ic.is_finite() && mean_ic.is_finite() && recent_ic < mean_ic * 0.5);
    FactorHealth {
        mean_ic,
        recent_ic,
        psi: drift,
