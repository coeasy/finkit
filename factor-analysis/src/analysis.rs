...
impl Default for WeightConfig {
    fn default() -> Self {
        Self { demeaned: true, group_adjust: None, equal_weight: false }
    }
}
...
fn normalize_group(values: &[(usize, f64)], out: &mut [f64], demeaned: bool, equal_weight: bool) {
    ...
    let mut scores: Vec<(usize, f64)> = values
        ...
        .map(|(row, value)| {
            ...
            if equal_weight {
                score = if score > 0.0 { 1.0 } else if score < 0.0 { -1.0 } else { 0.0 };
            }
            (*row, score)
        })
        .collect();
    if equal_weight && demeaned {
        ...
        for (_, score) in &mut scores {
            if *score > 0.0 && positives > 0.0 { *score /= positives; }
            if *score < 0.0 && negatives > 0.0 { *score /= negatives; }
        }
    }
    let gross = scores.iter().map(|(_, score)| score.abs()).sum::<f64>();
    if gross <= f64::EPSILON { return; }
    for (row, score) in scores { out[row] = score / gross; }
}

/// Build date-local factor weights. Group adjustment gives each group equal gross contribution.
pub fn factor_weights(frame: &ResearchFrame, factor_column: &str, config: &WeightConfig) -> ResearchResult<Vec<f64>> {
        ...
        if let Some(group_values) = groups {
            ...
            for row in range {
                if factor[row].is_finite() {
                    by_group.entry(group_values[row]).or_default().push((row, factor[row]));
                }
            }
            if by_group.is_empty() { continue; }
            ...
            for values in by_group.values() {
                ...
                for (row, _) in values { out[*row] = tmp[*row] * group_scale; }
            }
            let gross = range_weights_gross(&out, frame.index().date_segments().range(segment).unwrap());
            if gross > f64::EPSILON {
                for row in frame.index().date_segments().range(segment).unwrap() { out[row] /= gross; }
            }
        } else {
...
pub fn factor_returns(
    ...
    if weights.len() != frame.index().len() {
        return Err(ResearchError::LengthMismatch { name: "weights".to_string(), expected: frame.index().len(), actual: weights.len() });
    }
    let mut result = BTreeMap::new();
    for (&period, returns) in forward_returns {
        if returns.len() != frame.index().len() {
            return Err(ResearchError::LengthMismatch { name: format!("forward return {period}"), expected: frame.index().len(), actual: returns.len() });
        }
...
pub fn mean_information_coefficient(ic: &BTreeMap<usize, Vec<f64>>) -> BTreeMap<usize, f64> {
    ic.iter().map(|(&period, values)| {
        let finite: Vec<f64> = values.iter().copied().filter(|v| v.is_finite()).collect();
        let mean = if finite.is_empty() { f64::NAN } else { finite.iter().sum::<f64>() / finite.len() as f64 };
        (period, mean)
    }).collect()
}
...
pub fn mean_return_by_quantile(
    ...
    (1..=max_q).map(|q| {
        let by_period = forward_returns.iter().map(|(&period, values)| {
            let selected: Vec<f64> = values.iter().copied().zip(quantiles.iter().copied())
                .filter_map(|(value, bucket)| (bucket == q && value.is_finite()).then_some(value))
                .collect();
            let mean = if selected.is_empty() { f64::NAN } else { selected.iter().sum::<f64>() / selected.len() as f64 };
            (period, mean)
        }).collect();
        (q, by_period)
    }).collect()
...
pub fn quantile_turnover(frame: &ResearchFrame, quantiles: &[u16], quantile: u16, lag: usize) -> ResearchResult<Vec<f64>> {
    if quantiles.len() != frame.index().len() {
        return Err(ResearchError::LengthMismatch { name: "quantiles".to_string(), expected: frame.index().len(), actual: quantiles.len() });
    }
    if lag == 0 { return Err(ResearchError::InvalidConfig("turnover lag must be > 0".to_string())); }
    let sets: Vec<BTreeSet<AssetId>> = frame.index().date_segments().map(|range| {
        range.filter(|&row| quantiles[row] == quantile).map(|row| frame.index().assets()[row]).collect()
    });
    let mut out = vec![f64::NAN; sets.len()];
    for i in lag..sets.len() {
        if sets[i].is_empty() { out[i] = 0.0; continue; }
...
pub fn rank_autocorrelation(frame: &ResearchFrame, factor_column: &str, lag: usize) -> ResearchResult<Vec<f64>> {
    let factor = frame.column(factor_column)?;
    let snapshots: Vec<BTreeMap<AssetId, f64>> = frame.index().date_segments().map(|range| {
        ...
        rows.into_iter().zip(ranks).map(|(row, rank)| (frame.index().assets()[row], rank)).collect()
    });
    let mut out = vec![f64::NAN; snapshots.len()];
    for i in lag..snapshots.len() {
        let pairs: Vec<(f64, f64)> = snapshots[i].iter().filter_map(|(asset, &rank)| snapshots[i - lag].get(asset).map(|&old| (rank, old))).collect();
        if pairs.len() >= 2 {
...
pub fn factor_alpha_beta(
    ...
    factor_ret.iter().filter_map(|(&period, returns)| {
        let asset_returns = forward_returns.get(&period)?;
        let universe = frame.index().date_segments().map(|range| {
            let valid: Vec<f64> = range.map(|row| asset_returns[row]).filter(|v| v.is_finite()).collect();
            if valid.is_empty() { f64::NAN } else { valid.iter().sum::<f64>() / valid.len() as f64 }
        });
        let fit = simple_ols(returns, &universe).ok()?;
        Some((period, AlphaBeta { alpha: fit.intercept, beta: fit.coefficients.first().copied().unwrap_or(f64::NAN), r_squared: fit.r_squared }))
    }).collect()
}

/// Cumulative wealth by horizon from date-level factor returns.
pub fn cumulative_factor_returns(factor_ret: &BTreeMap<usize, Vec<f64>>) -> BTreeMap<usize, Vec<f64>> {
    factor_ret.iter().map(|(&period, values)| (period, cumulative_returns(values, 1.0))).collect()
}
...
mod tests {
    ...
    fn ic_detects_cross_sectional_signal() {
        let index = PanelIndex::new(vec![1,1,1,2,2,2], vec![AssetId(1),AssetId(2),AssetId(3),AssetId(1),AssetId(2),AssetId(3)]).unwrap();
        let mut frame = ResearchFrame::new(index);
        frame.add_numeric("factor","factor",vec![1.0,2.0,3.0,1.0,2.0,3.0]).unwrap();
        frame.add_numeric("price","market",vec![10.0,10.0,10.0,11.0,12.0,13.0]).unwrap();
        let forward = compute_forward_returns(&frame,"price",&ForwardReturnConfig::new(vec![1],ReturnKind::Arithmetic).unwrap()).unwrap();
